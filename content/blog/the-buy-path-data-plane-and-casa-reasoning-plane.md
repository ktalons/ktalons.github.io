...
Here's the honest part. 16 GB carries Phases A and B comfortably. Wazuh single-node in containers, tuned so the indexer never starves the manager, with headroom left for a fleeting recon job. It does **not** carry Phase C's Active Directory range or Phase D's honeynet-plus-OpenCTI stack. Those are RAM- and disk-hungry, and pretending otherwise would just set up a face-plant later.
...
The box lands Wednesday. Phase 0 is the cutover: Ubuntu, Docker, the stack green, backups verified. Then Phase A is real telemetry from real endpoints with dashboards on top. I'll post when it's live, with screenshots, not promises.
